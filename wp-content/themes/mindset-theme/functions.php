...
function fwd_setup() {
	...
	// Portait Blog Size = 200px width, 250px height, hard crop
	add_image_size( 'portrait-blog', 200, 250, true );
	add_image_size( 'preview-blog', 400, 200, true);
...
function fwd_widgets_init() {
	...
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-2',
			'description'   => esc_html__( 'Add more widgets here.', 'fwd' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title">',
			'after_title'   => '</h2>',
...
function fwd_theme_colour() {
	echo '<meta name="theme-color" content="#fff200">';
}
add_action( 'wp_head', 'fwd_theme_colour', 1 );

// Change excerpt length to 20 words
function fwd_excerpt_length( $length ) {
	return 20;
}
add_filter( 'excerpt length', 'fwd_excerpt_length', 999 );
